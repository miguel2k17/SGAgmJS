...
public class ServletControlador extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("listarPersonas".equals(accion)) {
            this.confirmarUsuarioEnSession(request, response, accion);
        } else if ("validarUsuario".equals(accion)) {
            this.validarUsuario(request, response,accion);
    ...
    private void confirmarUsuarioEnSession(HttpServletRequest request, HttpServletResponse response, String accion)
            throws ServletException, IOException {
        ...
        if (usuario != null) {
            if ("listarPersonas".equals(accion)) {
               ...
               //Si ya existe un usuario en session, lo redireccionamos a la lista de usuarios
    ...
    private void listarUsuarios(HttpServletRequest request, HttpServletResponse response)
        ...
        //Si el usuario ya esta en session, lo redireccionamos al listado de personas
        if (usuario != null) {
            UsuariosPojo userPojo = null;
            List<UsuariosPojo> listUsuariospojo = new ArrayList<UsuariosPojo>();
            ...
            for(Usuario u : usuarios){
                ...
                if(u.getIdPersona()!=null){
                   persona = personaService.getPersonaById(u.getIdPersona()); 
                }
                
                
                userPojo = new UsuariosPojo();
                userPojo.setUsuario(u);
                userPojo.setPersona(persona);
                listUsuariospojo.add(userPojo);
            }
            ...
            //}
            if(listUsuariospojo!=null && listUsuariospojo.size()>0){
                request.setAttribute("listaUsuariosPojo", listUsuariospojo);
            }
    ...
    private void validarUsuario(HttpServletRequest request, HttpServletResponse response,String accion)
            throws ServletException, IOException {
        ...
        if (usuarioValido) {
           ...
           // if ("listarPersonas".equals(accion)) {
            ...
            //}else{
              ...
              //  this.listarUsuarios(request, response); 
           // }
           //Redireccionamos a la pagina de index donde esta el menu con las opciones disponibles
    ...
    //Metodo agregar usuario, re direcciona a la pagina de Dettalle de Usuario
     private void agregarUsuario(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        
        //Obtenemos un listado con todas las personas y lo agregamos al
        //alcance de peticion
     ...
     //Este metodo nos permite insertar o modificar un Usuario
    //La diferencia va a estar si recibimos o no una llave primaria por parte del formulario
    private void guardarUsuario(HttpServletRequest request, HttpServletResponse response)
        ...
        Integer idPersonaParam = Integer.parseInt(request.getParameter("idPersona"));
        ...
        usuario.setIdPersona(idPersonaParam); //Si no venia en los parametros se coloca null
        usuario.setUsername(usuarioParam);
    ...
    //metodo que recupera una instancia del Usuario a editar y redirecciona a
    //la pagina de 'detalleUsuario.jsp'
     private void editarUsuario(HttpServletRequest request, HttpServletResponse response)
        ...
        if (idUsuarioParam != null && !idUsuarioParam.trim().equals("")) {
            ...
            //Utilizamos el servicio de Usuario para recuperar el objeto de la BD
            UsuarioService usuarioService = UsuarioServiceImpl.getInstance();
            Usuario usuario = usuarioService.getUsuarioById(idUsuario);

            //compartimos el objeto persona obtenido, para poderlo modificar
            request.setAttribute("usuarioItem", usuario);
            
            //Utilizamos el servicio de Persona para obtener todas las personas que hay
            //en la BD
            PersonaService personaService = PersonaServiceImpl.getInstance();
            List<Persona> personas = personaService.getAllPersonas();
            //Compartimos la lista obtenida
            request.setAttribute("listaPersonas", personas);
            request.getRequestDispatcher("/WEB-INF/pages/sga/detalleUsuario.jsp").forward(request, response);
